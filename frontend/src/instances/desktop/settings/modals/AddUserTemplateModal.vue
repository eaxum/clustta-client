      ...
      <div v-else class="category-area">
        <div class="category-list">
          <div v-for="(file, index) in selectedFiles" :key="index" class="file-item-wrapper">
            ...
            <InputAlert :show="duplicateNameIndices.includes(index)" :message="$t('modals.duplicateTemplateName')" />
          </div>
        </div>
...
const createTemplate = async () => {
  ...
  try {
    ...
    for (const file of selectedFiles.value) {
      await TemplateService.CreateTemplate(projectTemplateStore.activeProjectTemplate.uri, file.name, file.path);
    }

    await projectTemplateStore.reloadProjectTemplate();
