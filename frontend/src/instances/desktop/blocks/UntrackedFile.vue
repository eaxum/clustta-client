<template>
  <div ref="itemItem" class="item-main" v-right-click="openMenu" v-stop-propagation
    :class="{ 'item-main-selected': stage.markedItems.includes(item.id) }"
    @dblclick="launchTaskCommand(index, item, $event)">
    ...
    <div class="task-spacer" v-tooltip="'Untracted Item'" @click="console.log(item)">
      <span class="single-action-button single-action-button-disabled">
        <img class="small-icons entity-collapsed" src="/types-icons/other.svg">
      </span>
    </div>
    ...
    <div class="main-item-root">
      ...
      <div class="item-container drop-zone">
        ...
        <ActionButton :icon="getAppIcon('plus-circle')" v-tooltip="'Add File'" @click="importItem" />
...
const importItem = () => {
  ...
  const itemPath = props.item.file_path;

  dndStore.droppedFiles = []
  dndStore.targetItemId = parentId;
  dndStore.droppedFiles.push(itemPath);
