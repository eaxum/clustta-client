...
/**
 * Builds the full path for a conflict item (entity_path + name + trailing slash for entities).
 * @param {Object} conflict - The conflict object with entity_path, name, and type
 * @returns {string} The full path of the conflict item
 */
export function buildConflictFullPath(conflict) {
  const basePath = conflict.entity_path || '';
  const name = conflict.name || '';
  
  if (conflict.type === 'entity') {
    // Entities need trailing slash since they can contain children
    return `${basePath}${name}/`;
  }
  // Tasks don't need trailing slash
  return `${basePath}${name}`;
}
 ...
 * @param {Object} parent - The potential parent conflict (must be entity type)
 * @returns {boolean} True if child is nested under parent
 */
export function isChildOfConflict(child, parent) {
  if (parent.type !== 'entity') return false;
  if (child.local_id === parent.local_id) return false;
  
  const parentFullPath = buildConflictFullPath(parent);
  const childPath = child.entity_path || '';
  
  return childPath.startsWith(parentFullPath);
}

/**
 * Finds all child conflicts of a given parent entity conflict.
 * @param {Object} parent - The parent entity conflict
...
export function filterTopLevelConflicts(conflicts) {
  const entityConflicts = conflicts.filter(c => c.type === 'entity');
  
  return conflicts.filter(conflict => {
    // Check if any entity conflict is a parent of this conflict
    const hasConflictedParent = entityConflicts.some(
      entityConflict => isChildOfConflict(conflict, entityConflict)
    );
...
export function countAffectedItems(conflict, allConflicts) {
  if (conflict.type !== 'entity') return 1;
...
export function getResolutionSummary(conflict, allConflicts, action) {
  ...
  const typeName = conflict.type === 'entity' ? 'Collection' : 'Asset';
